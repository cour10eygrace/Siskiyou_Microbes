...
Code!="W07N04HC"& Code!="W07N06XA")#remove missing samples 
```
...
#library(rcompanion)
#transformTukey(funguild_abund_rich$mprich)#lambda=0.35

aov_mprich<-(aov((mprich^0.35)~ Hsuccess*Spp, funguild_abund_rich))
summary(aov_mprich)
TukeyHSD(aov_mprich)

#transformTukey(funguild_abund_rich$mpratio)#lambda 0.15

aov_mpratio<-aov((mpratio^0.15)~ Hsuccess*Spp, funguild_abund_rich)
summary(aov_mpratio)
...
```{r anovas and krusal wallace tests, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
kruskal.test(Basidiomycota~ successx , fphyreads)#sig
...
kruskal.test(Mortierellomycota~ Hsuccess , fphyreads)#sig
NemenyiTest(fphyreads$Mortierellomycota, as.factor(fphyreads$Hsuccess), dist="tukey") #Y < N
kruskal.test(Mortierellomycota~ successx , fphyreads)#sig
NemenyiTest(fphyreads$Mortierellomycota, as.factor(fphyreads$successx), dist="tukey") #Y < N
```
...
```{r, Appendix1, echo=FALSE,include=FALSE, message=FALSE, warning=FALSE}
...
bacterial_rel%>%mutate(tot=rowMeans(.))%>%select(tot)
...
fungal_rel%>%mutate(tot=rowMeans(.))%>%select(tot)
